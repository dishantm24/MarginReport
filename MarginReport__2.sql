...
round(convert(float, Pledge)-PledgeRequired + EPI_TDAY + NetLedgerT + Filler3 + [Funded Amount],2) as 'Assets'---AE5-AF5+AJ5+AY5+AM5+AP5
...
c.Filler5,c1.AddiAssetsforPeak,c1.LedgerT,c1.FNO,c1.CDS,c1.MCX,c1.NetLedgerT,c1.Assets,
...
((c.[EPI_T+2] + c1.cond4 + c.[ReceiptT+2] + c1.[Sum(BM:BP)])) AS 'CreditAvailableonT+2'
...
c.[EPI_T-1DAY],c.[EPIT(to reduce on T+2)],c.EPI_TDAY,c.[EPI_T+1],c.[EPI_T+2],c.Filler3,c.Payout_T,c.Filler4,c.[Funded Amount],c.[ Deriv T-1 Debit Bill],
c.Filler5,c1.AddiAssetsforPeak,c1.LedgerT,c1.FNO,c1.CDS,c1.MCX,c1.NetLedgerT,c1.Assets,
case when c1.Assets<0 OR   c1.Assets < c1.TotalT then c1.Assets
when c1.Assets<0 OR   c1.TotalT < c1.Assets then  c1.TotalT
else 0
end as 'Comparision',ISNULL(NULL,0) as 'NULLData',
Case when  (convert(float,c1.Pledge)-c1.PledgeRequired+ c1.EPI_TDAY + c1.NetLedgerT + c1.[Funded Amount] + c1.AddiAssetsforPeak) > 0 then
round(convert(float,c1.Pledge)-c1.PledgeRequired+ c1.EPI_TDAY + c1.NetLedgerT + c1.[Funded Amount] + c1.AddiAssetsforPeak,2)
else 0
end as 'AssetsForPeak',c.[NetCashBillT+1],c.[ReceiptT+1],c.DerivativeBillT,c.[CDST+1Bill],c.[MCXT+1Bill],c.[NCDEXT+1Bill]
...
Case when c.[TotallT+1] < 0 OR c2.[CreditAvailableonT+1] <c.[TotallT+1] then c2.[CreditAvailableonT+1]
 when c.[TotallT+1] < 0 OR  c.[TotallT+1] < c2.[CreditAvailableonT+1] then c.[TotallT+1]
 else 0 
...
from CTE1 c1  inner join CTE c on c1.ClientCode = c.ClientCode
inner join CTE2 c2 on c.ClientCode = c2.ClientCode



---=IF(AB5>0,MIN(BJ5,AB5),0)
